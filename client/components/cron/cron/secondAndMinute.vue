...
<template>
  <div :val="value_">
    <div>
      <el-radio v-model="type" label="1" size="default" border>{{ $t("每分") }}{{ lable }}</el-radio>
    </div>
    <div>
      <el-radio v-model="type" label="2" size="default" border>{{ $t("周期") }}</el-radio>
      <span style="margin-left: 10px; margin-right: 5px">{{ $t("从") }}</span>
      <el-input-number @change="type = '2'" v-model="cycle.start" :min="1" :max="59" size="default" style="width: 100px"></el-input-number>
      <span style="margin-left: 5px; margin-right: 5px">{{ $t("至") }}</span>
      <el-input-number @change="type = '2'" v-model="cycle.end" :min="2" :max="59" size="default" style="width: 100px"></el-input-number>
      {{ lable }}
    </div>
    <div>
      <el-radio v-model="type" label="3" size="default" border>{{ $t("循环") }}</el-radio>
      <span style="margin-left: 10px; margin-right: 5px">{{ $t("从") }}</span>
      <el-input-number @change="type = '3'" v-model="loop.start" :min="0" :max="59" size="default" style="width: 100px"></el-input-number>
      <span style="margin-left: 5px; margin-right: 5px">{{ lable }}{{ $t("开始") }}，{{ $t("每") }}</span>
      <el-input-number @change="type = '3'" v-model="loop.end" :min="1" :max="59" size="default" style="width: 100px"></el-input-number>
      {{ lable }}{{ $t("执行一次") }}
    </div>
    <div>
      <el-radio v-model="type" label="4" size="default" border>{{ $t("指定") }}</el-radio>
